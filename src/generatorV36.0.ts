...
export class ApiGenerator {
  ...
  private generateCode(apiDocs: any, framework: string, outputType: string): string {
    ...
    for (const [controllerName, methods] of controllers) {
      const description  = apiDocs.tags.find((tag: any) => tag.name === controllerName)?.description || '';
...
`;
  ...
  private generateTypeDefinition(name: string, schema: any, apiDocs: any): string {
    ...
    const propertyDefs = Object.entries(properties).map(([propName, propSchema]) => {
      ...
      const type = this.getTypeScriptType(propSchema as OpenAPIV3.SchemaObject, apiDocs);
      return `  /** ${(propSchema as OpenAPIV3.SchemaObject).description || (propSchema as OpenAPIV3.SchemaObject).title || ''} */\n  ${sanitizedPropName}${isRequired ? '' : '?'}: ${type};`;
    });
    ...
    if (operation.parameters) {
      operation.parameters.forEach((param: any) => {
        if (this.isParameterObject(param)) {
          ...
          const type = this.getTypeScriptType(param.schema || param, operation.components?.schemas || {});
    ...
    if (operation.requestBody) {
      ...
      if (content) {
        ...
        if (content['application/json']) {
          ...
          if (schema) {
            params.push(`body: ${this.getTypeScriptType(schema, operation.components?.schemas || {})}`);
          }
        }
        // 处理 multipart/form-data
        else if (content['multipart/form-data']) {
          ...
          if (schema) {
            params.push(`formData: ${this.getTypeScriptType(schema, operation.components?.schemas || {})}`);
          }
        }
        // 处理 application/x-www-form-urlencoded
        else if (content['application/x-www-form-urlencoded']) {
    ...
    if (operation.responses) {
      ...
      if (successResponse && successResponse.content) {
        ...
        if (successResponse.content['application/json']) {
          ...
          if (schema) {
            return this.getTypeScriptType(schema, operation.components?.schemas || {});
          }
        }
        // 检查其他内容类型
        const contentType = Object.keys(successResponse.content)[0];
