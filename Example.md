...
打开侧边栏中的 **配置中心** 后，页面顶部提供两张工作列表：

- **监听 API 配置**：同时展示 URL 和本地文件来源，可开启/关闭监听、设为默认或移除。
- **URL 缓存**：可将缓存加入监听列表、编辑名称与 URL，或删除缓存。

使用注意：

- `+` 新增 URL 配置时填写的名称会同步到 URL 缓存。
- URL 缓存已有名称时，加入监听列表会直接沿用，不会再次询问。
- 删除 URL 缓存不会删除已经加入的监听 API 配置。
- 移除监听配置或删除 URL 缓存前都会弹出确认提示。
