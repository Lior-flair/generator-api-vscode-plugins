...
- 配置中心新增“监听 API 配置”列表，统一展示 URL 与本地文件来源、监听状态和实际来源路径。
- 监听 API 配置支持开启/关闭监听、设置唯一默认配置，以及从当前工作区监听列表移除；移除前会进行确认。
- 配置中心新增“URL 缓存”列表，展示名称、URL 和文档版本，支持加入监听列表、编辑名称/URL 和删除缓存。
- 已加入监听列表的 URL 会显示当前状态，避免重复添加；删除 URL 缓存不会影响已经存在的监听配置。
- 新增 URL Profile 时会把名称同步到 URL 缓存；从具名缓存加入监听列表时直接沿用名称，不再重复询问。

#### 配置中心布局修复

- 修复生成参数筛选栏在编辑器宽度不足时把“显示高级配置”拆行的问题。
- 搜索框改为弹性占据剩余空间，“仅显示已配置”和“显示高级配置”保持在同一行。
- “移除”和“删除”操作使用错误色强调，降低与普通编辑操作混淆的概率。
